 ...
 ░░█████████  ░░███████  ████████ ░░██████  █████    ░░█████████  █████  █████      ░░█████ 
  ░░░░░░░░░    ░░░░░███ ░░░░░░░░   ░░░░░░  ░░░░░      ░░░░░░░░░  ░░░░░  ░░░░░        ░░░░░  
    ...
    def whois_lookup(self):
        ...
        whois_file = f"whois_{self.timestamp}.txt"
        ...
        if output:
            ...
            if self.verbose:
                ...
                preview = "\n".join(lines[:15])
                self.log(f"WHOIS preview:\n{preview}" + ("\n..." if len(lines) > 15 else ""), level="INFO")
        else:
            self.log("WHOIS lookup failed or no data returned.", level="WARNING")

    def nslookup_lookup(self):
        """Run NSLOOKUP to get full DNS records (ANY query)."""
        self.log(f"Starting NSLOOKUP for {self.target}...", level="INFO")
        nslookup_file = f"nslookup_{self.timestamp}.txt"
        nslookup_cmd = f"nslookup -query=any {self.target}"
    ...
    def dig_lookup(self):
        """Run DIG for full DNS records (no +short, clean output)."""
        self.log(f"Starting DIG lookup for {self.target}...", level="INFO")
        dig_file = f"dig_{self.timestamp}.txt"
        dig_cmd = f"dig {self.target} ANY +nocomments +noquestion +noauthority +noadditional"

        output = self.run_command(dig_cmd, dig_file)
        self.results["dig"] = output if output else ""

        if output:
            self.log("DIG lookup completed.", level="SUCCESS")
            if self.verbose:
                lines = [line for line in output.splitlines() if line.strip() and not line.startswith(';')]
                preview = "\n".join(lines[:15])
                self.log(f"DIG preview:\n{preview}" + ("\n..." if len(lines) > 15 else ""), level="INFO")
        else:
            self.log("DIG lookup failed or no data returned.", level="WARNING")

    def traceroute_scan(self):
        """Run traceroute with 3 probes per hop for accuracy."""
        self.log(f"Starting traceroute to {self.target}...", level="INFO")
        traceroute_file = f"traceroute_{self.timestamp}.txt"
        traceroute_cmd = f"traceroute -w 3 -q 3 {self.target}"

        output = self.run_command(traceroute_cmd, traceroute_file)
        self.results["traceroute"] = output if output else ""

        if output:
            hops = [line for line in output.splitlines() if re.match(r'^\s*\d+', line)]
            self.log(f"Traceroute completed. {len(hops)} hops traced.", level="SUCCESS")
            if self.verbose and hops:
                preview = "\n".join(hops[:15])
                self.log(f"Traceroute hops:\n{preview}" + ("\n..." if len(hops) > 15 else ""), level="INFO")
        else:
            self.log("Traceroute failed or no route found.", level="WARNING")

    def port_scanning(self):
        """Run Nmap for basic port scanning."""
        self.log(f"Starting port scanning for {self.target}...", level="INFO")
        nmap_file = f"nmap_{self.timestamp}.txt"
        ...
        if output:
            ...
            if self.verbose and open_ports:
                self.log("Open ports:\n" + "\n".join(open_ports[:15]) + ("\n..." if len(open_ports) > 15 else ""), level="INFO")
        else:
            self.log("Port scanning failed or returned no output.", level="WARNING")

    def subdomain_enumeration(self):
        """Run Subfinder, Amass, Sublist3r, and theHarvester for subdomain enumeration."""
        self.log(f"Starting subdomain enumeration for {self.target}...", level="INFO")
        subdomains = []

        # Subfinder
        subfinder_file = f"subfinder_{self.timestamp}.txt"
        ...
        if os.path.exists(subfinder_file_path):
            ...
            if self.verbose and subfinder_subdomains:
                preview = subfinder_subdomains[:15]
                self.log("Sample Subfinder subdomains:\n" + "\n".join(preview) + ("\n..." if len(subfinder_subdomains) > 15 else ""), level="INFO")
        else:
            self.log("Subfinder enumeration failed or no results.", level="WARNING")

        # Amass
        amass_file = f"amass_{self.timestamp}.txt"
        ...
        if os.path.exists(amass_file_path):
            ...
            if self.verbose and amass_subdomains:
                preview = amass_subdomains[:15]
                self.log("Sample Amass subdomains:\n" + "\n".join(preview) + ("\n..." if len(amass_subdomains) > 15 else ""), level="INFO")
        else:
            self.log("Amass enumeration failed or no results.", level="WARNING")

        # Sublist3r
        sublist3r_file = f"sublist3r_{self.timestamp}.txt"
        ...
        if sublist3r_output:
            ...
            if self.verbose and sublist3r_subdomains:
                preview = sublist3r_subdomains[:15]
                self.log("Sample Sublist3r subdomains:\n" + "\n".join(preview) + ("\n..." if len(sublist3r_subdomains) > 15 else ""), level="INFO")
        else:
            self.log("Sublist3r enumeration failed or no results.", level="WARNING")

        # theHarvester
        theharvester_file = f"theharvester_{self.timestamp}.txt"
        theharvester_file_path = os.path.join(self.output_dir, theharvester_file)
        theharvester_cmd = f"theHarvester -d {self.target} -b all -f {theharvester_file_path}"
        
        self.run_command(theharvester_cmd, capture=False)
        if os.path.exists(theharvester_file_path):
            with open(theharvester_file_path, "r") as f:
                theharvester_output = f.read().strip()
            theharvester_subdomains = [line.strip() for line in theharvester_output.splitlines() if line.strip() and '.' in line and self.target in line and not line.startswith(('-', '*', '['))]
            subdomains.extend(theharvester_subdomains)
            self.log(f"Found {len(theharvester_subdomains)} subdomains with theHarvester.", level="SUCCESS")
            if self.verbose and theharvester_subdomains:
                preview = theharvester_subdomains[:15]
                self.log("Sample theHarvester subdomains:\n" + "\n".join(preview) + ("\n..." if len(theharvester_subdomains) > 15 else ""), level="INFO")
        else:
            self.log("theHarvester enumeration failed or no results.", level="WARNING")

        subdomains = list(set(subdomains))
        self.results["subdomains"] = subdomains
        self.log(f"Total unique subdomains found: {len(subdomains)}.", level="SUCCESS")
        if self.verbose and subdomains:
            preview = subdomains[:15]
            self.log("Sample combined subdomains:\n" + "\n".join(preview) + ("\n..." if len(subdomains) > 15 else ""), level="INFO")
    ...
    def directory_enumeration(self):
        ...
        dirb_file = f"dirb_{self.timestamp}.txt"
        ...
        if os.path.exists(dirb_file_path):
            ...
            if self.verbose and directories:
                preview = directories[:15]
                self.log("Sample directories:\n" + "\n".join(preview) + ("\n..." if len(directories) > 15 else ""), level="INFO")
        else:
    ...
    def nuclei_scan(self):
        ...
        nuclei_file = f"nuclei_{self.timestamp}.txt"
        ...
        if os.path.exists(nuclei_file_path):
            ...
            if self.verbose and nuclei_results:
                preview = nuclei_results[:15]
                self.log("Sample Nuclei findings:\n" + "\n".join(preview) + ("\n..." if len(nuclei_results) > 15 else ""), level="INFO")
        else:
    ...
    def nikto_scan(self):
        ...
        nikto_file = f"nikto_{self.timestamp}.txt"
        ...
        try:
            with open(report_file, "w") as f:
                ...
                if self.results["nslookup"]:
                    f.write(self.results["nslookup"] + "\n\n")
                else:
                    f.write("No NSLOOKUP data available.\n\n")

                # DIG
                f.write(f"DIG DNS Records:\n")
                f.write(f"{'-'*30}\n")
                if self.results["dig"]:
                    lines = [l for l in self.results["dig"].splitlines() if l.strip() and not l.startswith(';')]
                    f.write("\n".join(lines) + "\n\n")
                else:
                    f.write("No DIG data available.\n\n")

                # Traceroute
                f.write(f"Traceroute Path:\n")
                f.write(f"{'-'*30}\n")
                if self.results["traceroute"]:
                    hops = [line for line in self.results["traceroute"].splitlines() if re.match(r'^\s*\d+', line)]
                    f.write("\n".join(hops) + "\n\n")
                else:
                    f.write("Traceroute failed or no path found.\n\n")
...
def main():
    parser = argparse.ArgumentParser(description="CyberSift: Full Recon with WHOIS, NSLOOKUP, DIG, Traceroute, Nmap, Subfinder, Amass, Sublist3r, Dirb, Nuclei, and Nikto")
